...
```{r}
meth.chisq <- methdata.summary %>% filter(feature=="gene" | feature=="exon" | feature=="intron" | feature=="2kbflank-up" | feature=="2kbflank-down" | feature=="TE" | feature=="ASV" | feature=="unknown") %>% column_to_rownames("feature") %>% select(CpGs, methylated) %>% chisq.test() #no, they are different. P-value very low. 
meth.chisq
...
--> Exons and splice variants are more likely to be methylated. 
...
--> Transposable elements, introns, and gene flanking regions are less likely. 
...
The relative contribution of each cell to the total Chi-square score give some indication of the nature of the dependency between rows and columns of the contingency table. From the table/image above, it can be seen that the most contributing cells to the Chi-square are ASV/methylated (47.54%), unknown/methylated (26.6%), CDS & exons / methylated (5.6% each). 
