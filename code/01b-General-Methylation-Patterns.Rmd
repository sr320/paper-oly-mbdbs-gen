...
# Save to objects 
save(allmeth, file="../analyses/methylation-characteristics/R-objects/allmeth")
save(allmeth.5x, file="../analyses/methylation-characteristics/R-objects/allmeth.5x")
...
save(methdata.summary.long, file="../analyses/methylation-characteristics/R-objects/methdata.summary.long")
```
...
left_join(
  fread(here::here("analyses", "methylation-characteristics", "methylated-gene2kb.bed"), sep = "\t", dplyr::select=c(5,8,9)) %>% 
  count(V5, V8, V9) %>%  setNames(c("contig_gene", "start_gene", "end_gene", "n_mCpG")),
  
  fread(here::here("analyses", "methylation-characteristics", "CpGs-gene2kb.bed"), dplyr::select=c(10,13,14)) %>% 
...
methylated.genes <- read_delim(here::here("analyses", "methylation-characteristics", "methylated-gene.bed"), delim = '\t', col_names = F) %>% as_tibble() %>% 
...
CpGs.genes <- read_delim(here::here("analyses", "methylation-characteristics", "CpGs-gene.bed"), delim = '\t', col_names = F) %>% as_tibble() %>%
...
read_delim(file="../analyses/methylation-characteristics/methylated-loci-enriched-BP.txt", delim = "\t") %>% 
...
read_delim(file="../analyses/methylation-characteristics/methylated-loci-enriched-BP.txt", delim = "\t") %>%
  ...
  write.csv("../analyses/methylation-characteristics/methylated-loci-enriched-BP.csv",quote = F,row.names = F, na = "NA")
```
