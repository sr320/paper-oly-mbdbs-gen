...
## 3. Population-specific loci that are methylated overlapping with genes 

```{bash}
bedtools intersect -wb -a "../analyses/methylation-filtered/HC-meth-loci.bed" -b "../genome-features/Olurida_v081-20190709.gene.gff" >  ../analyses/methylation-filtered/HC-meth-gene.bed

bedtools intersect -wb -a "../analyses/methylation-filtered/SS-meth-loci.bed" -b "../genome-features/Olurida_v081-20190709.gene.gff" >  ../analyses/methylation-filtered/SS-meth-gene.bed

bedtools intersect -wb -a "../analyses/methylation-filtered/all-meth-loci.bed" -b "../genome-features/Olurida_v081-20190709.gene.gff" >  ../analyses/methylation-filtered/all-meth-gene.bed
```
...
DML.features[["DML.gene2kb"]] %>%
  mutate(Name=str_remove(Name, "Name=")) %>% mutate(Name=str_remove(Name, ";")) %>%  #remove extraneous info from Olur gene ID
  left_join(Olurida_gene_uniprot, by=c("Name" = "geneID")) %>%  #add uniprot IDs to gene dataframe
  select(uniprotID) %>% na.omit() %>% as.vector() %>%  # select only uniprot ID column, remove NA values and convert to vector 
...
| Category         | Term                                        | RT | Count | %   | P-Value  | Fold Enrichment | Benjamini | FDR      | Fisher Exact |
|------------------|---------------------------------------------|----|-------|-----|----------|-----------------|-----------|----------|--------------|
| GOTERM_BP_DIRECT | glucose homeostasis                         | RT | 4     | 5.3 | 1.10E-02 | 7.9             | 1.00E+00  | 1.50E+01 | 1.00E-03     |
| GOTERM_BP_DIRECT | positive regulation of cell   proliferation | RT | 5     | 6.7 | 2.10E-02 | 4.5             | 1.00E+00  | 2.70E+01 | 3.90E-03     |
| GOTERM_BP_DIRECT | digestive tract development                 | RT | 3     | 4   | 3.20E-02 | 9.9             | 1.00E+00  | 3.80E+01 | 2.20E-03     |
| GOTERM_BP_DIRECT | kidney development                          | RT | 4     | 5.3 | 3.50E-02 | 5.3             | 9.90E-01  | 4.10E+01 | 5.40E-03     |
| GOTERM_BP_DIRECT | rhythmic process                            | RT | 3     | 4   | 4.40E-02 | 8.4             | 9.90E-01  | 4.80E+01 | 3.80E-03     |
| GOTERM_BP_DIRECT | receptor-mediated endocytosis               | RT | 4     | 5.3 | 4.90E-02 | 4.6             | 9.90E-01  | 5.20E+01 | 8.70E-03     |
| GOTERM_BP_DIRECT | Notch signaling pathway                     | RT | 4     | 5.3 | 5.70E-02 | 4.4             | 9.90E-01  | 5.70E+01 | 1.10E-02     |
| GOTERM_BP_DIRECT | liver development                           | RT | 3     | 4   | 7.10E-02 | 6.6             | 9.90E-01  | 6.60E+01 | 8.40E-03     |
...
### No Enriched biological function, SALs. Here are the genes: 

| UNIPROT_ACCESSION | Name                                                | Species                 |
|-------------------|-----------------------------------------------------|-------------------------|
| Q9NUV9            | GTPase, IMAP family member 4(GIMAP4)                | Homo sapiens            |
| Q3HNG7            | H1 histone family, member O, oocyte-specific(H1FOO) | Bos taurus              |
| Q9DC40            | telomere maintenance 2(Telo2)                       | Mus musculus            |
| Q6IMN6            | caprin family member 2(CAPRIN2)                     | Homo sapiens            |
| Q5SZD4            | glycine-N-acyltransferase like 3(GLYATL3)           | Homo sapiens            |
| Q0E908            | Hillarin(Hil)                                       | Drosophila melanogaster |
| P79152            | Cytochrome P450 3A19                                | Capra hircus aegagrus   |
| P18503            | Short-chain collagen C4                             | Ephydatia muelleri      |

### Compare all genes in HC samples to all genes genome, then all genes in SS to all genes in genome. 

# These are genes which contain methylated loci in HC & SS samples: 
../analyses/methylation-filtered/HC-meth-gene.bed
../analyses/methylation-filtered/SS-meth-gene.bed
...
# Copy UniprotIDs for all genes that contain 5 or greater methylated loci in the Hood Canal 
read_delim(here::here("analyses", "methylation-filtered", "HC-meth-gene.bed"), delim = '\t', col_names = FALSE) %>% #read in list of methylated loci in HC population that fall within genes
...
read_delim(here::here("analyses", "methylation-filtered", "SS-meth-gene.bed"), delim = '\t', col_names = FALSE) %>% #read in list of methylated loci in HC population that fall within genes
mutate(Gene=str_extract(X12, "ID=(.*?);")) %>% mutate(Gene=str_remove(Gene, "ID=")) %>% mutate(Gene=str_remove(Gene, ";")) %>% #extract only O. lurida gene ID 
  select(Gene) %>% count(Gene) %>%  #Select only gene ID column, count number of methylated loci in each gene 
  filter(n>4) %>% #filter for only genes that contain 5 or more methylated loci 
  left_join(Olurida_gene_uniprot, by=c("Gene" = "geneID")) %>% #join the UniprotID information to the list of O.lurida gene IDs 
...
DAVID results saved here: 
- Enriched biological functions in Hood Canal, methylated genes: ../analyses/methylation-filtered/DAVID_enriched-methylated-genes-HC.txt
- Enriched biological functions in South Sound, methylated genes: ../analyses/methylation-filtered/DAVID_enriched-methylated-genes-SS.txt  
