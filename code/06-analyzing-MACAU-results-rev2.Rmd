...
# Re-do analysis with MACAU results from unite() filter option where min.per.group = 8 

That is, MACAU results which analyzed count data for loci where 8 of the 9 of samples in each population had **10x coverage** or greater. 
...
"../analyses/macau/output/20200205-macau.assoc.txt"
MACAU.results <- read.table(file=here::here("analyses","macau","output","20200205-macau.assoc.txt"), sep = "\t", stringsAsFactors = F)
colnames(MACAU.results) <- MACAU.results[1,]
MACAU.results <- MACAU.results[-1, ] 
MACAU.results$pvalue <- as.numeric(MACAU.results$pvalue)
hist(MACAU.results$pvalue)

summary(MACAU.results$id == counts.filtered.tot$siteID) #confirm that macau results and original destranded dataframe are in same order 
MACAU.results.c <- cbind(counts.filtered.tot[,1],MACAU.results) %>%
   separate(`counts.filtered.tot[, 1]`, into = c("contig", "locus"), sep = "_")
write.table(MACAU.results.c, file = here::here("analyses", "macau", "MACAU.results.c.tab"), sep = "\t", row.names = FALSE,  quote = FALSE)
...
```{r}
hist(MACAU.results.c$pvalue) # p-value distribution looks good (i.e. not u-shaped)
macau.qvalue <- qvalue(p = MACAU.results.c$pvalue, fdr.level = 0.1) # create qvalue object, setting FDR=10%
...
plot(macau.qvalue)
summary(macau.qvalue$significant) #13 sign. (# TRUE)
macau.FDR <- cbind(MACAU.results.c, do.call(cbind.data.frame, macau.qvalue[c("qvalues", "pvalues", "significant")]))
...
nrow(macau.sign) #13
macau.sign.meth.counts <- merge(x=macau.sign, y=subset(counts.filtered.meth, siteID %in% macau.sign$id), by.x="id", by.y="siteID")
macau.sign.tot.counts <- merge(x=macau.sign, y=subset(counts.filtered.tot, siteID %in% macau.sign$id), by.x="id", by.y="siteID")
write.table(macau.sign.meth.counts, file = here::here("analyses", "combined-macau-FDR-meth-counts.txt"), sep = "\t", row.names = FALSE,  quote = FALSE) 
write.table(macau.sign.tot.counts, file = here::here("analyses", "combined-macau-FDR-tot-counts.txt"), sep = "\t", row.names = FALSE,  quote = FALSE) 
```

### Create df where coverage counts <5 replaced with NA, create % methylated dataframe with sign. MACAU loci 

```{r}
macau.sign.tot.counts.na <- macau.sign.tot.counts %>% 
  mutate_at(vars(contains('coverage')), funs(ifelse(. < 5, NA, .)))
macau.sign.perc.meth <- macau.sign.meth.counts
macau.sign.perc.meth[,20:37] <- 100*(macau.sign.meth.counts[,20:37]/macau.sign.tot.counts.na[,20:37])
```
...
macau.sign.perc.meth <- macau.sign.perc.meth %>%
  mutate(mean.HC = rowMeans(macau.sign.perc.meth[,20:28])) %>%
  mutate(mean.SS = rowMeans(macau.sign.perc.meth[,29:37])) %>%
  mutate(HC.SS.ratio = mean.HC/mean.SS) %>%
  arrange(HC.SS.ratio)
rownames(macau.sign.perc.meth) <- macau.sign.perc.meth[,1]

par(cex.main=.8)
heatmap.2(as.matrix(macau.sign.perc.meth[, grep(c("numCs"), colnames(macau.sign.perc.meth))]), 
          main = "MACAU id'd % methylated\n5x cov\nOrdered by cluster analysis", 
          xlab = NA, ylab = NA, 
          hclustfun = function(x) hclust(x, method="ward.D"), distfunc <- function(x) daisy(x,metric="correlation"), trace="none", dendrogram = "column", na.color = "white",
          col = colorRampPalette(brewer.pal(4,"YlGnBu")), colCol = c(rep("firebrick3", times=9), rep("dodgerblue3", times=9)))
...
heatmap.2(as.matrix(macau.sign.perc.meth[, grep(c("numCs"), colnames(macau.sign.perc.meth))]), 
         ...
         na.color = "white",trace="none", dendrogram = "none", Rowv=FALSE, Colv = FALSE,
          col = colorRampPalette(brewer.pal(4,"YlGnBu")), colCol = pop.colors.tree)
View(macau.sign.perc.meth)
par(cex.main=.8)
heatmap.2(as.matrix(macau.sign.perc.meth[, size.macau$sample]), 
          main = "MACAU id'd % methylation\nFDR=10%\nOrdered by Length", 
          xlab = NA, ylab = NA, 
          trace="none", dendrogram = "none", Rowv=FALSE, Colv = FALSE,
          col = colorRampPalette(brewer.pal(4,"YlGnBu")), colCol = pop.colors.length)

par(oma=c(0,0,5,0))
barplot(size.macau[order(size.macau$y.MBD.FILENAME),]$y.Length,  names.arg = size.macau[order(size.macau$y.MBD.FILENAME),]$sample, col = mycols[size.macau[order(size.macau$y.MBD.FILENAME),]$pop], cex.names =  0.8, main = "Shell Length (mm)", las=2, space=1)
```

### Create BED files for 1) MACAU id'd loci with 5x coverage, and 2) all loci with 5x coverage  

```{r}
macau.sign.perc.meth %>% 
  mutate(end = locus) %>% 
  rename(start = locus) %>%
  dplyr::select(c("contig", "start", "end")) %>%
write_delim(here::here("analyses","macau", "macau.sign.perc.meth.bed"),  delim = '\t', col_names = FALSE)
