...
View(allLoci.features.df["Ontology_term"])
...
GO.MWU.GOterms <- allLoci.features.df %>% filter(feature == "gene") %>%
  ...
  select(ID, Ontology_term) 

write.table(GO.MWU.GOterms, here::here("analyses", "GO_MWU_GO-terms"),sep="\t",quote = F,row.names = F, col.names=F)
```
...
write.table(GO.MWU.signif, here::here("analyses", "GO_MWU_signif"),sep="\t",quote = F,row.names = F, col.names=F)
```
