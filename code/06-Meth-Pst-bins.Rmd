...
# NOTE: None of the 1kb genome bin analyses were used in the manuscript - only used 10kb bins. 
...
intersectBed \
  ...
  > "../analyses/methylation/meth_filter_1kb-bin.tab"
```

### Count various things 

```{bash}
wc -l "../analyses/methylation/meth_filter_long.tab"
wc -l "../analyses/methylation/meth_filter_1kb-bin.tab" 
...
```{bash}
head -n 5  "../analyses/methylation/meth_filter_1kb-bin.tab" 
```


```{r}
meth_filter_bins_1kb <- read_delim(file = "../analyses/methylation/meth_filter_1kb-bin.tab", delim = "\t", col_names = c(colnames(meth_filter_reshaped), "contig", "bin.start", "bin.end")) %>% 
...
ncol(perc_meth_bins_1kb_wide)-1 # 281 bins
...
save(perc_meth_bins_1kb_Pst, file="../analyses/methylation/R-objects/perc_meth_bins_1kb_Pst")
...
write.table(perc_meth_bins_1kb_Pst %>% 
            ...
            file = here::here("analyses", "methylation", "Pst_bins_1kb.tab"), 
            sep = '\t', na = "NA", row.names = FALSE, col.names = TRUE, quote=FALSE)
```

## 10kb bins 

### Use intersectBed to find where methylated loci and 10kb genome bins (that have genetic data)

  wb: Print all lines in the second file
  a: input data file, which was created in previous code chunk
  b: save annotated gene list

The file Olurida_v081_bins-10kb.bed was created by Katherine Silliman using the Oly genome index file (Olurida_v081.fa.fai) and this code: 
```
...
intersectBed \
  ...
  > "../analyses/methylation/meth_filter_10kb-bin.tab"
```

```{r}
meth_filter_bins_10kb <- read_delim(file = "../analyses/methylation/meth_filter_10kb-bin.tab", delim = "\t", col_names = c(colnames(meth_filter_reshaped), "contig", "bin.start", "bin.end")) %>% 
  mutate(bin=paste(contig, bin.start, bin.end, sep="_"))
...
# How many of the 10kb bins have data at 5+ MBD loci? 
# UPDATE 10/4 - not gonna use this filter 
perc_meth_bins_10kb %>% 
  filter(`n()`>4) %>% ungroup() %>%
  distinct(bin) %>%
  nrow() #476 10kb bins 
...
perc_meth_bins_10kb_wide <- perc_meth_bins_10kb %>% 
  ungroup() %>%
  #filter(`n()`>4) %>% # filter for bins with 5+ loci if desired
  #tidyr::unite("gene_region", c("contig_gene", "start_gene_2kb", "end_gene_2kb"), sep = "_", remove = FALSE) %>%
  dplyr::select(population, sample, bin, mean_percMeth) %>%
  spread(bin, mean_percMeth) %>%
  tibble::column_to_rownames(var = "sample")

#Now run the following line and it will provide Pst estimates for every 10kb bins (n=827) for which we have methylation data AND genetic data.
...
round(mean(perc_meth_bins_10kb_Pst$Pst_Values), digits = 3)
round(sd(perc_meth_bins_10kb_Pst$Pst_Values), digits = 3)
nrow(perc_meth_bins_10kb_Pst)

# Write out Pst results 
save(perc_meth_bins_10kb_Pst, file=here::here("analyses", "methylation", "R-objects", "perc_meth_bins_10kb_Pst"))
write.table(
  perc_meth_bins_10kb_Pst %>% 
    separate(Quant_Varia, into=c("contig","start.bin", "end.bin"), sep = "_", remove = FALSE), 
  file = here::here("analyses", "methylation", "Pst_bins_10kb.tab"), 
  sep = '\t', na = "NA", row.names = FALSE, col.names = TRUE, quote=FALSE)
```
...
## Assess 1kb bins 
# NOTE: NOT RE-DONE IN THIS NEW ANALYSIS B/C IT TAKES TOO LONG AND NOT USED IN PAPER 
...
intersectBed \
  ...
  > "../analyses/methylation/meth_filter_1kb-bin-allmeth.tab"
```

### Import 1kb bin information for all methylation data (not just those that are in bins w/ genetic data)  

```{r}
meth_filter_bins_allmeth_1kb <- read_delim(file = "../analyses/methylation/meth_filter_1kb-bin-allmeth.tab", delim = "\t", col_names = c(colnames(meth_filter_reshaped), "contig", "bin.start", "bin.end")) %>% 
  mutate(bin=paste(contig, bin.start, bin.end, sep="_"))
```
...
# Save 1kb Pst object to file for 
save(perc_meth_bins_allmeth_1kb_Pst, file="../analyses/methylation/R-objects/perc_meth_bins_allmeth_1kb_Pst")
...
write.table(perc_meth_bins_allmeth_1kb_Pst %>% 
              separate(Quant_Varia, into=c("contig","start.bin", "end.bin"), sep = "_", remove = FALSE), 
            file = here::here("analyses", "methylation", "Pst_bins_allmeth_1kb.tab"), 
            sep = '\t', na = "NA", row.names = FALSE, col.names = TRUE, quote=FALSE)
```

## Assess 10kb bins 

### How many 10kb bins does the O. lurida genome have? Answer- 214,511 bins 

```{bash}
wc -l "../genome-features/Olurida_v081_10kbIntervals.bed"
```

### Use intersectBed to find where methylated loci and all 10kb genome bins intersect
```{bash}
intersectBed \
  -wb \
  -a "../analyses/methylation/meth_filter_long.tab" \
  -b "../genome-features/Olurida_v081_10kbIntervals.bed" \
  > "../analyses/methylation/meth_filter_10kb-bin-allmeth.tab"
```

### Import 10kb bin information for all methylation data (not just those that are in bins w/ genetic data)  

```{r}
meth_filter_bins_allmeth_10kb <- read_delim(file = "../analyses/methylation/meth_filter_10kb-bin-allmeth.tab", delim = "\t", col_names = c(colnames(meth_filter_reshaped), "contig", "bin.start", "bin.end")) %>% 
  mutate(bin=paste(contig, bin.start, bin.end, sep="_"))
```
...
```{r}
meth_filter_bins_allmeth_10kb %>% ungroup() %>%
       distinct(bin) %>%
  nrow()
```
...
# NOTE: Also (NEW AS OF 9/19/2021) I will only keep bins that have at minimum 5 loci. How many is that?  Answer: 14,088
perc_meth_bins_allmeth_10kb %>% filter(`n()`>4) %>%
  ungroup() %>%
       distinct(bin) %>% nrow()

# Reshape data. Need to have one row per sample, one column with the population, and separate columns with each gene region with % methylation. 
perc_meth_bins_allmeth_10kb_wide <- perc_meth_bins_allmeth_10kb %>% filter(`n()`>4) %>%
  ungroup() %>%
  #tidyr::unite("gene_region", c("contig_gene", "start_gene_2kb", "end_gene_2kb"), sep = "_", remove = FALSE) %>%
  dplyr::select(population, sample, bin, mean_percMeth) %>%
  spread(bin, mean_percMeth) %>%
...
### Run Pst for 10kb bins (all bins that contain 5+ loci with methylation data, not just bins with genetic+meth data)
...
save(perc_meth_bins_allmeth_10kb_Pst, file="../analyses/methylation/R-objects/perc_meth_bins_allmeth_10kb_Pst")
...
write.table(perc_meth_bins_allmeth_10kb_Pst %>% 
              separate(Quant_Varia, into=c("contig","start.bin", "end.bin"), sep = "_", remove = FALSE), 
            file = here::here("analyses", "methylation", "Pst_bins_allmeth_10kb.tab"), 
            sep = '\t', na = "NA", row.names = FALSE, col.names = TRUE, quote=FALSE)
```
