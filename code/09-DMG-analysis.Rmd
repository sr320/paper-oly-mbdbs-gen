...
list.of.packages <- c("reshape2","dplyr", "tidyr", "readr", "stringr", "plotly", "car", "Pstat") #add new libraries here 
...
for (i in 1:nrow(DMLs.in.DMGs_2kbslop)) {
  test <- DMLs.in.DMGs_2kbslop.calcs %>% 
    filter(chr==DMLs.in.DMGs_2kbslop$contig_gene[i] &
             start==DMLs.in.DMGs_2kbslop$start_gene[i]+1)
  DMLs_in_DMGs_2kbslop_plots[[i]] <- ggplot(test, aes(x = population, y = mean_percMeth, fill = population, 
...
head(perc_meth_genes_2kbslop) #current format 
head(perc.meth.Pst2) #desired format for Pst calculations
...
head(perc_meth_genes_2kbslop_wide) #confirm correct format 
...
perc_meth_genes_wide <- perc_meth_genes %>% 
  ...
  spread(gene_region, mean_percMeth) %>%
  column_to_rownames(var = "sample")
head(perc_meth_genes_wide) #confirm correct format 
