...
```{r}
meth_filter_genes <- 
  read_delim(file = here::here("analyses", "meth_filter_gene.tab"), delim = "\t", col_names = c(colnames(meth_filter_reshaped[-10]), "contig_gene", "source_gene", "feature_gene", "start_gene", "end_gene", "unknown1_gene", "strand_gene", "unknown2_gene", "notes_gene")) %>%
...
#checking to make sure numCs + numTs = coverage
plot(meth_filter_genes$numCs + meth_filter_genes$numTs ~ meth_filter_genes$coverage)  
...
ggplotly(meth_filter_genes %>%
  ...
  ggplot(aes(x = population, y = mean_cov, fill = population)) + geom_bar(stat="identity") + facet_wrap(~gene) + theme_light() + scale_fill_manual(values=c("firebrick3","dodgerblue3")))

DMG_counts <- meth_filter_genes %>%
  filter(contig_gene %in% DMGs.genes$contig_gene & 
           start_gene %in% DMGs.genes$start_gene & 
           end_gene %in% DMGs.genes$end_gene)
...
for (i in 1:length(DMG_genes_unique)) {
  temp <-  DMG_counts %>% 
  filter(gene == DMG_genes_unique[i]) %>%
  ...
  summarise(allCs_percent = 100*(sum(numCs)/sum(coverage)), 
    ...
    facet_wrap(~start) + 
    theme_light() + ggtitle(paste("gene = ", DMG_genes_unique[i], sep="")) +
