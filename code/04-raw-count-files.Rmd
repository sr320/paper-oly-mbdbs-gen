...
```{r}
counts.tot <- meth_filter.df[, grep(c("siteID|coverage"), colnames(meth_filter.df))]
```

Create dataframe with methylated counts  

```{r}
counts.meth <- meth_filter.df[, grep(c("siteID|numCs"), colnames(meth_filter.df))]
```
...
```{r}
counts.tot[is.na(counts.tot)] <- 0
counts.meth[is.na(counts.meth)] <- 0
