...
nrow(macau.20190812.05)
```

Extract raw count data for loci where p<0.05 in MACAU 

```{r}
head(macau.20190812.05.counts <- merge(x=macau.20190812.05[c("id", "pvalue")], y=subset(counts.meth.destrand, siteID %in% macau.20190812.05$id), by.x="id", by.y="siteID"))
nrow(macau.20190812.05.counts)

# there's a 3 row difference - why? 
```

Generate heatmap with only loci & counts where MACAU identified DML, using p<0.05 

---> mbd samples #1-9 = Hood Canal population  
---> mbd samples #10-18 = South Sound population  

```{r}
heatmap(as.matrix(macau.20190812.05.counts[c(-1:-2)]), main = "MACAU pvalue <0.05", xlab = "mbd sample", ylab = "loci")
```

Heatmap with loci where p-value<0.01 from MACAU

```{r}
nrow(macau.20190812.01 <- subset(macau.20190812, pvalue<0.01))
heatmap(as.matrix(merge(x=macau.20190812.01[c("id", "pvalue")], y=subset(counts.meth.destrand, siteID %in% macau.20190812.01$id), by.x="id", by.y="siteID")[c(-1:-2)]), main = "MACAU pvalue <0.01", xlab = "mbd sample", ylab = "loci")
```

Heatmap with loci where p-value<0.001 from MACAU

```{r}
nrow(macau.20190812.001 <- subset(macau.20190812, pvalue<0.001))
pdf(file = "analyses/macau/20190812-heatmap-p001.pdf")
print(heatmap.001 <- heatmap(as.matrix(merge(x=macau.20190812.001[c("id", "pvalue")], y=subset(counts.meth.destrand, siteID %in% macau.20190812.001$id), by.x="id", by.y="siteID")[c(-1:-2)]), main = "MACAU pvalue <0.001", xlab = "mbd sample", ylab = "loci"))
dev.off()

# Check out "top 100" - 100 loci with lowest p-values 
View(head(macau.20190812.001[order(macau.20190812.001$pvalue, decreasing = FALSE),], n=100))
```
