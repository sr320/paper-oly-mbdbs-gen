...
meth.destranded.df[, grep(c("siteID|coverage"), colnames(meth.destranded.df))]


Convert all NA values to 0 for both total count and methylated count files 

```{r}
meth_filter.df.zeros <- meth_filter.df %>% replace(is.na(.), 0)
```


```{r}
counts.tot <- meth_filter.df.zeros[, grep(c("siteID|coverage"), colnames(meth_filter.df.zeros))]
```

Create dataframe with methylated counts  

```{r}
counts.meth <- meth_filter.df.zeros[, grep(c("siteID|numCs"), colnames(meth_filter.df.zeros))]

table(counts.tot$siteID == counts.meth$siteID)
nrow(counts.tot)==nrow(counts.meth)

#counts.tot[-1] <- data.frame(lapply(counts.tot[-1], as.integer))
#counts.meth[-1] <- data.frame(lapply(counts.meth[-1], as.integer))
```
...
```{r}
write.table(meth_filter.df, file = "../analyses/meth_filter.df.txt", sep = "\t", row.names = FALSE,  quote = FALSE)
```
