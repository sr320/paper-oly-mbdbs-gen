...
Ran MACAU in a Jupyter Notebook. 

Read in dataframe with MACAU assocation results 

```{r}
macau.length <- read.table(file="analyses/macau/output/20190809-macau-1.assoc.txt", sep = "\t", stringsAsFactors = F)
head(macau.length)
colnames(macau.length) <- macau.length[1,]
macau.length <- macau.length[-1, ] 
```
