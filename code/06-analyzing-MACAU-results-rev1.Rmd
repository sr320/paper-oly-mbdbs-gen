...
#pdf(file = here::here("..", "analyses","test-heatmap.pdf"))
par(cex.main=.8)
...
macau.sign.perc.meth.10x %>%
  dplyr::select(c("id", "start", "end", "strand")) %>%
write_delim(here::here("analyses","macau-any10x.bed"),  delim = '\t', col_names = FALSE)

macau.sign.perc.meth.10x[complete.cases(macau.sign.perc.meth.10x[22:39]), ] %>%
  dplyr::select(c("id", "start", "end", "strand")) %>%
write_delim(here::here("analyses","macau-all10x.bed"),  delim = '\t', col_names = FALSE)
```
